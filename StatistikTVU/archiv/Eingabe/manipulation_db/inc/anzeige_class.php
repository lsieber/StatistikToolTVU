...
class Anzeige{
    ...
    public function set_mitglied(){
        ...
        while($row=mysql_fetch_assoc($rmitglied)) {
            ...
            $this->mit[$counter][5]= 1000;        // Platz für Kategorie 
    ...
    public function get_bestenliste(){
        include ('../../inc/connect.inc.php');

        $rbest=mysql_query(
            "SELECT b.ID, b.Leistung, b.Mitglied, b.DisziplinID,
            m.Vorname, m.Name, m.Jg, 
            w.WKname, w.Jahr,
            d.Disziplin
            FROM bestenliste b
            ...
            ORDER BY Vorname, Wettkampf, DisziplinID ");
        if (! $rbest){
        echo"query mistake" . mysql_error();
        }

        $counter = 0;
        while($row=mysql_fetch_array($rbest,MYSQL_NUM)) 
        {
            $this->best[$counter] = $row; 
            $counter++;
        }
        $this->bestsize=$counter; // Anzahl der Disziplinen=Grösse Matrix
